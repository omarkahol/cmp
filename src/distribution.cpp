...
double cmp::distribution::MultivariateNormalDistribution::logPDF(const Eigen::Ref<const Eigen::VectorXd> &x) const {
    return logPDF(mean_ - x, ldltDecomposition_);
}

double cmp::distribution::MultivariateNormalDistribution::logJumpPDF(const Eigen::Ref<const Eigen::VectorXd> &jump) {
...
double cmp::distribution::MultivariateNormalDistribution::dLogPDF(const Eigen::Ref<const Eigen::VectorXd> &res, const Eigen::LDLT<Eigen::MatrixXd> &ldltDecomposition, const Eigen::Ref<const Eigen::MatrixXd> &cov_gradient, const Eigen::Ref<const Eigen::VectorXd> &mean_gradient) {
...
double cmp::distribution::MultivariateNormalDistribution::ddLogPDF(const Eigen::Ref<const Eigen::VectorXd> &res, const Eigen::LDLT<Eigen::MatrixXd> &ldltDecomposition, const Eigen::Ref<const Eigen::MatrixXd> &cov_gradient_l, const Eigen::Ref<const Eigen::MatrixXd> &cov_gradient_k, const Eigen::Ref<const Eigen::MatrixXd> &cov_hessian) {
...
void cmp::distribution::MultivariateNormalDistribution::set(const Eigen::Ref<const Eigen::VectorXd> &x) {
    mean_ = x;
}
...
double cmp::distribution::UniformDistribution::sample(std::default_random_engine &rng) {
    return  distN_(rng) * (upperBound_ - lowerBound_);
    + lowerBound_;
}
...
double cmp::distribution::MultivariateStudentDistribution::logPDF(const Eigen::Ref<const Eigen::VectorXd> &res, const Eigen::LDLT<Eigen::MatrixXd> &ldltDecomposition, const double &nu) {
    size_t dim = res.size();
    Eigen::VectorXd alpha = ldltDecomposition.solve(res); // Sigma^{-1} * x
...
double cmp::distribution::MultivariateUniformDistribution::logPDF(const Eigen::Ref<const Eigen::VectorXd> &x) const {
    for(size_t i = 0; i < x.size(); i++) {
        if(x(i) < mean_(i) - size_(i) * 0.5 || x(i) > mean_(i) + size_(i) * 0.5) {
            return -std::numeric_limits<double>::infinity();
        }
    }
    return -x.size() * std::log(size_.prod());
}

double cmp::distribution::MultivariateUniformDistribution::logJumpPDF(const Eigen::Ref<const Eigen::VectorXd> &jump) {
...
void cmp::distribution::MultivariateUniformDistribution::set(const Eigen::Ref<const Eigen::VectorXd> &x) {
    mean_ = x;
}
...
double cmp::distribution::NormalInverseWishartDistribution::logPDF(const Eigen::Ref<const Eigen::VectorXd>& x) const {
    ...
    return cmp::distribution::MultivariateStudentDistribution::logPDF(res, covLDLT_, nu_);
}


Eigen::VectorXd cmp::distribution::NormalInverseWishartDistribution::sample(
    std::default_random_engine& rng) {
    // This class represents the NIW prior predictive over x, which is a
    // multivariate Student-t: x ~ St(μ = mean_, Σ = covLDLT_, ν = nu_).
    // So we sample directly from that Student-t:
    // 1) y ~ N(0, Σ)
    // 2) s ~ ChiSq(ν)
    // 3) x = μ + y / sqrt(s/ν)

    // Build a normal draw with covariance given by covLDLT_ via its LDLT factors
    const auto &L = covLDLT_.matrixL();
    const auto &P = covLDLT_.transpositionsP();
    const Eigen::VectorXd D = covLDLT_.vectorD();

    // z ~ N(0, I), scale by sqrt(D)
    Eigen::VectorXd z = Eigen::VectorXd::Zero(dim_);
    for(size_t i = 0; i < dim_; ++i) {
        z(i) = distN_(rng) * std::sqrt(std::abs(D(i)));
    }

    // y = P^T * L * z has covariance Σ
    Eigen::VectorXd y = P.transpose() * (L * z);

    // One shared chi-square draw across all dimensions
